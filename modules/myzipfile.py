...
rupor: This is modified zipfile.py from Lib - preserve original file name if no UTF-8 has been specified - legacy encoding
"""
import io
import os
import re
...
class ZipInfo (object):
    ...
    def __init__(self, filename="NoName", date_time=(1980,1,1,0,0,0)):
        ...
        self.filename_raw = b'' # rupor: keep unmodified filename here
    ...
    def _encodeFilenameFlags(self):
        if len(self.filename_raw) > 0:
            # rupor: leave original name intact
            return self.filename_raw, self.flag_bits
        else:
            try:
...
class ZipFile:
    ...
    def _RealGetContents(self):
        ...
        while total < size_cd:
            ...
            if self.debug > 2:
                print(centdir)
            filename_raw = fp.read(centdir[_CD_FILENAME_LENGTH])
            flags = centdir[5]
            if flags & 0x800:
            ...
            x = ZipInfo(filename)
            if not flags & 0x800:
                # rupor: preserve unmodified filename
                x.filename_raw = filename_raw
